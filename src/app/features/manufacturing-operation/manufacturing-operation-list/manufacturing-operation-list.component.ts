...
export class ManufacturingOperationListComponent {
  ...
  loadLookups(): void { this.service.getCustomers(1, 500).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(r => this.customers.set((r.data as PagedResponse<Customer> | undefined)?.items ?? [])); this.service.getManufacturingItems(1, 500).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(r => this.items.set((r.data as PagedResponse<ManufacturingItem> | undefined)?.items ?? [])); }
  ...
  exportCsv(): void { const rows = [['Date', 'Item Code', 'Customer', 'Type', 'Qty', 'Rejected Qty', 'Rejected Reason', 'Challan No', 'Lot No', 'Remarks'], ...this.operations().map(x => [this.formatDate(x.operationDate), x.itemCode, x.customerCode, x.operationType, String(x.quantity), String(x.rejectedQuantity || 0), x.rejectedReason || '', x.challanNo || '', x.lotNo || '', x.remarks || ''])]; const csv = rows.map(r => r.map(v => `"${v.replace(/"/g, '""')}"`).join(',')).join('\n'); const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); const a = document.createElement('a'); a.href = url; a.download = 'manufacturing-operations.csv'; a.click(); URL.revokeObjectURL(url); }
