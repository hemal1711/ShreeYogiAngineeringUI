...
export class FastenerFormComponent {
  ...
  onSubmit(): void { if (this.form.invalid) { Object.values(this.form.controls).forEach(c => c.markAsTouched()); this.toastService.warning('Please complete required fields.', 'Fastener needs attention'); return; } const id = this.correlationId(); const value = this.form.getRawValue(); const request: FastenerRequest = { itemCode: value.itemCode ?? '', itemName: value.itemName ?? '', category: value.category ?? undefined, size: value.size ?? undefined, unit: value.unit ?? undefined, currentStock: value.currentStock ?? 0, minimumStock: value.minimumStock ?? 0, shopName: value.shopName ?? undefined, location: value.location ?? undefined, isActive: value.isActive ?? true, photoUrl: this.currentPhotoUrl() ?? undefined }; this.isSubmitting.set(true); const op = id ? this.service.updateFastener(id, request, this.selectedPhoto()) : this.service.createFastener(request, this.selectedPhoto()); op.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => { this.isSubmitting.set(false); this.toastService.success('Fastener saved successfully.', 'Fastener saved'); this.router.navigate(['/fasteners']); }, error: e => { this.isSubmitting.set(false); this.toastService.error(e?.error?.message || 'We could not save this fastener.', 'Save failed'); } }); }
